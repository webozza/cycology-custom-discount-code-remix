...
export const loader = async ({ request }: LoaderFunctionArgs) => {
  ...
  const response = await admin.graphql(DISCOUNTS_QUERY, {
    variables: { first: 2, query: "type:app" }, // app-managed only
  });
...
export default function Index() {
  ...
  const hasNext = Boolean(discounts?.pageInfo?.hasNextPage && discounts?.pageInfo?.endCursor);
  const endCursor = discounts?.pageInfo?.endCursor ?? null; 
